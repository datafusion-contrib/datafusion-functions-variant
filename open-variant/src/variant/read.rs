...
#[derive(Clone)]
pub struct VariantRef<'a>(pub &'a [u8]);
...
impl<'a> VariantRef<'a> {
    ...
    pub fn get_primitive_type_id(&self) -> PrimitiveTypeId {
        let header = self.0[0];
        (header >> 2).try_into().expect("Invalid PrimitiveTypeId")
    }

    pub fn get_bool(&self) -> bool {
        match self.get_primitive_type_id() {
    ...
    pub fn get_i64(&self) -> i64 {
        if !matches!(self.get_primitive_type_id(), PrimitiveTypeId::Int64) {
            panic!("Not an i64");
        }
    ...
    pub fn get_f64(&self) -> f64 {
        if !matches!(self.get_primitive_type_id(), PrimitiveTypeId::Float64) {
            panic!("Not an f64");
        }
    ...
    pub fn get_string<'b>(&'b self) -> &'a str {
        if !matches!(self.get_primitive_type_id(), PrimitiveTypeId::String) {
            panic!("Not a string");
        }
    ...
    pub fn get_object_value<'b>(&'b self, field_id: usize) -> Option<VariantRef<'a>> {
        let object_ref = ObjectRef::try_new(self).ok()?;
        object_ref.get_field(field_id)
    }

    pub fn get_array_element(&self, index: usize) -> Option<VariantRef<'a>> {
        let array_ref = ArrayRef::try_new(self).ok()?;
        array_ref.get_element(index)
    }
}
...
impl<'a> ObjectRef<'a> {
    ...
    pub fn try_new(data: &VariantRef<'a>) -> Result<Self, String> {
        if !matches!(data.get_basic_type(), BasicType::Object) {
...
impl<'a> ArrayRef<'a> {
    pub fn try_new(data: &VariantRef<'a>) -> Result<Self, String> {
        if !matches!(data.get_basic_type(), BasicType::Array) {
